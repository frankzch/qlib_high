...
class Alpha158DL(QlibDataLoader):
    ...
    def get_feature_config(
        ...
        if "rolling" in config:
            windows = config["rolling"].get("windows", [5, 10, 20, 30])
            ...
            if use("VSUMD"):
                ...
                # steady rise
            #    fields += [
            #        "If(Or(($close - Ref($close, %d))/$close/%d > 0.04, ($close - Ref($close, %d))/$close/%d < 0.01), 0, 1/(Std(($close/Ref($close, 1)-1), %d) + 1e-12))"
                    #"If((1000> 100 | 1< 10), 0, 1/(Std(($close/Ref($close, 1)-1),%d) + 1e-12))"
            #        % (d, d, d, d, d)
            #        for d in windows
            #    ]
            #    names += ["SDRS%d" % d for d in windows]

        return fields, names
        
class Alpha158DLCrypto(QlibDataLoader):
    """Dataloader to get Alpha158"""
